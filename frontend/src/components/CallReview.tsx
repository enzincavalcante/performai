...
export function CallReview() {
  ...
  if (status === "report" && report && documentMode) {
    ...
    return <div className="call-review-page crm-report-page">
      ...
      <article className="crm-document">
        ...
        {report.competencies.length > 0 && <section><h2>10. Avaliacao por competencia</h2><div className="crm-competencies">{report.competencies.map((item)=><article key={item.name}><header><strong>{item.name.replaceAll("_"," ")}</strong><b>{item.score ?? "--"}/10</b></header><p>{item.feedback ?? "Analise detalhada nao fornecida."}</p>{item.nextStep && <small><b>Como melhorar:</b> {item.nextStep}</small>}</article>)}</div></section>}
  ...
  if (status === "report" && report) return <div className="call-review-page">
    ...
    {report.evaluationBlocks.length > 0 && <section className="review-section evaluation-blocks"><div className="review-section-title"><Target /><div><p className="eyebrow">AVALIACAO COMPLETA</p><h2>Analise criteriosa em {report.evaluationBlocks.length} criterios</h2></div></div><div>{report.evaluationBlocks.map((block,index)=><article key={block.name}><header><span>{String(index+1).padStart(2,"0")}</span><h3>{block.name}</h3><strong>{block.score ?? "--"}<small>/10</small></strong></header><div>{block.reason && <p className="evaluation-reason"><b>Motivo da nota</b>{block.reason}</p>}<p><b>O que funcionou</b>{block.worked ?? "A IA nao encontrou evidencia segura deste comportamento."}</p><p><b>O que precisa melhorar</b>{block.improve ?? "A IA nao encontrou evidencia segura deste comportamento."}</p>{block.how && <p><b>Como melhorar</b>{block.how}</p>}{block.example && <p><b>Exemplo pratico</b>{block.example}</p>}{block.excerpt && <blockquote>&ldquo;{block.excerpt}&rdquo;</blockquote>}</div></article>)}</div></section>}
    {report.competencies.length > 0 && <section className="review-section"><div className="review-section-title"><Target /><div><p className="eyebrow">COMPETENCIAS</p><h2>Desempenho por habilidade</h2></div></div><div className="review-skills">{report.competencies.map((item) => <article key={item.name}><header><strong>{item.name.replaceAll("_", " ")}</strong>{item.score !== undefined && <span>{item.score}/10</span>}</header>{item.score !== undefined && <div><i style={{ width: `${Math.max(0, Math.min(100, item.score * 10))}%` }} /></div>}{item.level && <em>{item.level}</em>}{item.feedback && <p>{item.feedback}</p>}{item.impact && <p><b>Impacto:</b> {item.impact}</p>}{item.gap && <p><b>Para evoluir:</b> {item.gap}</p>}{item.nextStep && <p><b>Proximo passo:</b> {item.nextStep}</p>}</article>)}</div></section>}
