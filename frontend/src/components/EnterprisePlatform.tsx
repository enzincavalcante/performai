...
function CertificatesModule({ onNavigate }: { onNavigate: Navigate }) {
  ...
  const [participantName, setParticipantName] = useState(() => typeof window === "undefined" ? "" : window.localStorage.getItem("performai_certificate_name") || "");
  ...
  return <ModuleFrame eyebrow="CERTIFICACAO PROFISSIONAL" title="Uma carreira comprovada, nao apenas um curso." description="O primeiro certificado exige 1 mes e meio (6 semanas) de treinamento na Performa AI. Depois, novos niveis validam sua evolucao a cada 3 meses." action={<div className="hub-actions"><button onClick={() => setShowPreview((value) => !value)}><Award /> {showPreview ? "Proteger previa" : "Visualizar certificado"}</button>{showPreview && <button onClick={() => window.print()}><FileText /> Salvar previa em PDF</button>}</div>}>
    <section className="certificate-levels">{[["1 MES E MEIO","Especialista em Vendas Consultivas","Formacao inicial de 6 semanas · fundamentos, pratica e avaliacao"],["3 MESES","Sales Professional","Consistencia comprovada em treino e calls reais"],["6 MESES","Sales Advanced","Dominio de diagnostico, objecoes e negociacao"],["9 MESES","Sales Expert","Alta performance, previsibilidade e influencia no time"],["12 MESES","Performa AI Master","Nivel maximo · excelencia sustentada e lideranca"]].map((level,index)=><article className={index===0?"current":""} key={level[0]}><small>{level[0]}</small><strong>{level[1]}</strong><span>{level[2]}</span>{index===0?<b>EM ANDAMENTO</b>:<Lock />}</article>)}</section>
    <section className="certificate-lock"><div className="certificate-lock-visual"><span><Lock /></span><p>CICLO INICIAL · 1 MES E MEIO</p><h2>Especialista em Vendas Consultivas</h2><small>A emissao definitiva em PDF sera liberada somente depois de 6 semanas e quando todos os criterios teoricos e praticos chegarem a 100%.</small><label className="certificate-name-field"><span>Nome completo no certificado</span><input value={participantName} onChange={(event)=>{setParticipantName(event.target.value);window.localStorage.setItem("performai_certificate_name",event.target.value);}} placeholder="Digite o nome completo" /></label><div className="enterprise-progress"><i style={{ width: `${readiness}%` }} /></div><strong>{readiness}% pronto para certificacao</strong></div><div className={`certificate-paper ${readiness < 100 ? "certificate-preview" : ""}`}><header><Image src={BRAND_LOGO} alt="Performa AI" width={42} height={42} /><strong>Performa <b>AI</b></strong><small>CERTIFICADO DE CONCLUSAO</small></header><p>Certificamos que</p><h2>{participantName.trim() || "Nome do participante"}</h2><p>concluiu 1 mes e meio de formacao <b>Especialista em Vendas Consultivas</b>, demonstrando dominio teorico e pratico das competencias comerciais avaliadas.</p><footer><div><Image src={FOUNDER_SIGNATURE} alt="Assinatura do fundador" width={180} height={75} /><i /><strong>Enzo Cavalcante</strong><small>Fundador, Performa AI</small></div><span><ShieldCheck /><b>VALIDACAO DIGITAL</b><small>PERFORMA-2026-0001</small></span></footer>{!showPreview && <em><Lock /> Previa protegida ate a conclusao</em>}</div><div className="certificate-requirements">{requirements.map(([label, value, progress]) => <article key={label}><header><span>{label}</span><strong>{value}</strong></header><div className="enterprise-progress"><i style={{ width: `${progress}%` }} /></div></article>)}</div></section>
