...
function buildEvidenceRubricReport(transcript: string) {
  ...
  const enoughEvidence = clean.length >= 80 && sellerRows.length >= 1;
  ...
  const criteria: Criterion[] = [
    { name: "Abertura", weight: 5, signals: /bom dia|boa tarde|tudo bem|obrigad|prazer|tempo disponivel|objetivo da conversa/, improve: "Abra com contexto, objetivo e confirmacao de tempo.", example: "Obrigado pelo tempo. Proponho entender o cenario e combinar um proximo passo. Funciona para voce?" },
    { name: "Rapport", weight: 5, signals: /entendo|faz sentido|voce comentou|pelo que disse|obrigad/, improve: "Reconheca uma fala concreta do cliente antes de avancar.", example: "Entendi o ponto que voce trouxe. Quero aprofundar isso antes da solucao." },
    { name: "Diagnostico", weight: 9, signals: /como funciona|como fazem|cenario atual|hoje|processo|desafio|problema|impacto|consequencia/, improve: "Investigue contexto, causa, impacto e prioridade antes de apresentar.", example: "Como funciona hoje, onde trava e o que acontece quando esse problema continua?" },
    { name: "Qualidade das perguntas", weight: 7, signals: /\?/, improve: "Use perguntas abertas e aprofunde a resposta anterior.", example: "O que torna isso prioritario agora e como voces medem o impacto?" },
    { name: "Escuta ativa", weight: 7, signals: /entao.*voce|pelo que entendi|se entendi|voce disse|correto|faz sentido/, improve: "Parafraseie, confirme e use a resposta para decidir a proxima pergunta.", example: "Pelo que entendi, o impacto principal e a falta de previsibilidade. Correto?" },
    { name: "Identificacao da necessidade", weight: 8, signals: /necessidade|precisa|prioridade|dor|problema central|resultado esperado|objetivo/, improve: "Confirme a necessidade na linguagem do cliente e separe sintoma de causa.", example: "Entre esses problemas, qual precisa ser resolvido primeiro e por que agora?" },
    { name: "Pitch", weight: 7, signals: /por isso|com base|nesse cenario|para resolver|proposta|solucao|beneficio/, negative: /temos dashboard|temos automacao|varias funcionalidades/, improve: "Conecte somente capacidades relevantes a uma necessidade confirmada.", example: "Como voce relatou perda de previsibilidade, a proposta e reduzir esse ponto com acompanhamento mensuravel." },
    { name: "Comunicacao", weight: 5, signals: /entendi|resumindo|em outras palavras|vou explicar|objetiv/, improve: "Use frases curtas, transicoes claras e confirme entendimento.", example: "Vou resumir em uma frase e voce me diz se representa o que precisa." },
    { name: "Clareza", weight: 5, signals: /primeiro|depois|em resumo|ponto central|proximo passo|significa/, improve: "Organize a fala em uma ideia, uma evidencia e uma pergunta.", example: "O ponto central e X; a evidencia e Y. Isso faz sentido no seu cenario?" },
    { name: "Dominio do produto", weight: 5, signals: /funciona|implantacao|integracao|capacidade|limite|inclui|nao inclui/, negative: /nao sei|acho que funciona|talvez tenha/, improve: "Explique aplicacao, limites e aderencia sem prometer o que nao foi comprovado.", example: "Para este cenario, a capacidade aplicavel e X; o limite conhecido e Y." },
    { name: "Criacao de valor", weight: 8, signals: /case|cliente semelhante|dados|resultado|retorno|roi|econom|prova|impacto/, improve: "Conecte valor a impacto e evidencia verificavel do cliente.", example: "Qual indicador seria valido para comprovar resultado neste projeto?" },
    { name: "Tratamento de objecoes", weight: 7, signals: /entendo a preocupacao|o que esta por tras|alem do preco|se resolvermos|objecao|resistencia/, improve: "Valide, esclareca, responda com evidencia e confirme se resolveu.", example: "Alem do valor, existe algum risco ou criterio que ainda impede a decisao?" },
    { name: "Negociacao", weight: 6, signals: /contrapartida|condicionado|escopo|volume|prazo de pagamento|troca/, negative: /desconto.*(agora|hoje)|posso dar.*desconto/, improve: "Nao conceda preco antes de diagnosticar valor e obter contrapartida.", example: "Posso avaliar uma condicao se alinharmos prazo, escopo e compromisso." },
    { name: "Postura", weight: 5, signals: /transparente|nao consigo prometer|limite|confirmar|posso entender/, negative: /garantido|certeza absoluta|sem risco|voce esta errado/, improve: "Mantenha firmeza, empatia e transparencia sem promessas absolutas.", example: "Vou separar o que e comprovado, os limites e como o resultado sera medido." },
    { name: "Fechamento", weight: 6, signals: /faz sentido avancar|podemos fechar|o que falta|decisao|compromisso|assinatura/, improve: "Teste compromisso e confirme o que ainda impede a decisao.", example: "O que ainda precisa estar claro para avancarmos?" },
    ...
    { name: "Follow-up quando aplicavel", weight: 0, signals: /follow.?up|retorno|retomar|envio.*resumo|confirmo.*email|volto a falar/, improve: "Defina mensagem, canal, responsavel e prazo do acompanhamento.", example: "Envio o resumo hoje e confirmo na terca se os criterios foram validados." },
  ];
  const blocks = criteria.map((criterion) => {
    ...
    const questionBonus = criterion.name === "Qualidade das perguntas" ? Math.min(35, questionCount * 7) : 0;
    const sequenceBonus = matches.some((row) => {
      const rowIndex = rows.indexOf(row);
      return rowIndex >= 0 && rows.slice(rowIndex + 1, rowIndex + 3).some((next) => next.speaker !== row.speaker && next.text.length > 18);
    }) ? 8 : 0;
    const notApplicable = criterion.name === "Follow-up quando aplicavel" && !/follow.?up|retorno|retomar|email|resumo|proxima/.test(normalized);
    const score = enoughEvidence && !notApplicable ? Math.max(20, Math.min(100, 32 + matches.length * 18 + questionBonus + sequenceBonus - negatives.length * 24)) : null;
    const evidence = matches[0] ?? negatives[0];
    const missingEvidence = `A sequencia completa nao mostrou evidencia suficiente. Para elevar esta nota seria necessario observar: ${criterion.improve.toLowerCase()}`;
    return { name: criterion.name, weight: criterion.weight, score, reason: score === null ? (notApplicable ? "Este criterio nao se aplica com seguranca ao trecho analisado." : insufficient) : matches.length ? `Nota sustentada por ${matches.length} comportamento(s) explicito(s), pela sequencia da conversa e pela reacao posterior do cliente quando disponivel.` : missingEvidence, what_worked: matches.length ? `Comportamento observado em ${matches.map((row) => row.timestamp).slice(0, 3).join(", ")}.` : "Nao houve evidencia positiva explicita; a nota-base reflete a ausencia do comportamento esperado na sequencia completa.", what_to_improve: criterion.improve, how_to_improve: "Pratique o exemplo, aplique em simulacao e compare o mesmo criterio na proxima call.", practical_example: criterion.example, excerpt: evidence ? `[${evidence.timestamp}] ${evidence.text}` : missingEvidence };
  });
  ...
  const overall100 = totalWeight ? evaluable.reduce((sum, block) => sum + block.score * block.weight, 0) / totalWeight : 0;
  const descending = [...evaluable].sort((a, b) => b.score - a.score);
  const ascending = [...descending].reverse();
  const strengths = descending.filter((block) => block.score >= 60).slice(0, 3).map((block) => ({ title: block.name, evidence: block.excerpt, why_it_worked: block.reason, how_to_repeat: "Repita o comportamento e conecte-o ao proximo movimento da conversa." }));
  ...
  const criticalMoments = blocks.filter((block) => /^\[\d{2,}:\d{2}\]/.test(block.excerpt)).slice(0, 6).map((block) => ({ timestamp: block.excerpt.match(/^\[(\d{2,}:\d{2})\]/)?.[1] ?? "00:00", text: block.excerpt.replace(/^\[[^\]]+\]\s*/, ""), type: Number(block.score) >= 60 ? "positive" : "improvement", insight: `${block.name}: ${block.reason}`, recommendation: block.practical_example }));
  const topStrength = descending[0];
  const topGap = ascending[0];
  const questionCount = sellerRows.filter((row) => row.text.includes("?")).length;
  return {
    overall_score: Math.round(overall100),
    summary: enoughEvidence ? `A call recebeu ${overall100.toFixed(0)}/100 na matriz ponderada de 17 competencias, considerando a sequencia completa, comportamentos e evidencias por criterio.` : insufficient,
    diagnosis: { executive_summary: enoughEvidence ? `Desempenho de ${overall100.toFixed(0)}/100. Principal forca: ${topStrength?.name}. Prioridade: ${topGap?.name}.` : insufficient, call_objective: "O objetivo nao ficou explicito; para confirma-lo seria necessario observar uma declaracao de agenda ou resultado esperado.", conversation_context: `${rows.length} falas processadas e ${questionCount} perguntas atribuidas ao vendedor.`, seller_conduction: seller ? `${seller.speaker} foi tratado como vendedor por concentrar perguntas e sinais comerciais.` : insufficient, overall_diagnosis: topGap ? `${topGap.name} e a prioridade de desenvolvimento.` : insufficient, missed_opportunities: topGap?.what_to_improve ?? insufficient, missing_questions: "Investigue impacto, custo da inacao, decisao, prazo e compromisso final.", objections_analysis: blocks.find((block) => block.name === "Tratamento de objecoes")?.reason ?? insufficient, better_approach: topGap?.practical_example ?? insufficient, professional_conclusion: "Compare as mesmas 17 competencias nas proximas calls para medir evolucao real." },
    strengths,
    improvements,
    competency_scores: blocks.map((block) => ({ name: block.name, score: block.score, explanation: block.reason, impact: block.weight ? `Peso de ${block.weight}% na nota.` : "Avaliado somente quando aplicavel, sem alterar a nota geral.", level: block.score === null ? "Nao aplicavel" : block.score >= 80 ? "Avancado" : block.score >= 60 ? "Intermediario" : "Em desenvolvimento", gap: block.what_to_improve, next_step: block.practical_example })),
...
- competency_scores com EXATAMENTE as mesmas 17 competencias de evaluation_blocks. Cada objeto deve conter name, score de 0 a 100, explanation, impact, level, gap e next_step;
- next_actions com EXATAMENTE 3 objetos contendo priority, objective, practical_action, exercise, target e expected_result;
- evaluation_blocks com EXATAMENTE 17 itens. Cada item deve ter name, weight, score de 0 a 100, reason, what_worked, what_to_improve, how_to_improve, practical_example e excerpt;
- use estes 17 criterios, nesta ordem: Abertura; Rapport; Diagnostico; Qualidade das perguntas; Escuta ativa; Identificacao da necessidade; Pitch; Comunicacao; Clareza; Dominio do produto; Criacao de valor; Tratamento de objecoes; Negociacao; Postura; Fechamento; Proximo passo; Follow-up quando aplicavel.
